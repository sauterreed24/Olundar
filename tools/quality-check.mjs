...
check('canvas renderer keeps premium tactical sprites readable', () => {
  ...
  assert(indexSource.includes('Standards mark key moves') && indexSource.includes('Laurels mark supplied ground') && indexSource.includes('Hover previews route cost') && indexSource.includes('Red rim marks attack reach'), 'The compact battlefield hint should explain tactical overlays without opening another panel.');
  ...
  assert(mainSource.includes('function scrollOpeningFollowThroughRail') && mainSource.includes("['scout', 'contact', 'front']") && mainSource.includes('scrollOpeningFollowThroughRail(previousStepId)'), 'Scout and contact follow-through should scroll the next command rail into view after selecting the next actor.');
  ...
  assert(styleSource.includes('.turn-report') && styleSource.includes('.turn-report-metrics') && styleSource.includes('.turn-report-notes') && styleSource.includes('.turn-report-detail-drawer summary') && styleSource.includes('.turn-report-glance'), 'Turn reports need readable strategy-card styling with compact mobile disclosure.');
