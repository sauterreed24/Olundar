...
check('canvas renderer keeps premium tactical sprites readable', () => {
  ...
  assert(renderSource.includes('function drawMovementCommandGrid') && renderSource.includes('function drawMovementCommandCornerPins') && renderSource.includes('drawMovementCommandGrid(ctx, layout, reachable, maxMove, hoverMove);'), 'Selected units should show a crisp legal-move grid and pinned terrain-cost frontier instead of vague hover-only movement feedback.');
