...
check('canvas renderer keeps premium tactical sprites readable', () => {
  ...
  assert(mainSource.includes('function selectionStatMarkup') && mainSource.includes('function orderButton') && mainSource.includes('campaign-tools-drawer'), 'The command rail should expose structured stat chips, hierarchy, and quieter campaign utilities.');
  assert(styleSource.includes('.command-stats') && styleSource.includes('.stat-chip') && styleSource.includes('.order-button.primary') && styleSource.includes('.campaign-tools-drawer'), 'The command rail should look like a polished imperial control surface rather than generic form buttons.');
