...
check('first-turn guide gives actionable live onboarding', () => {
  ...
  assert(firstGuide.title === 'Opening Orders', 'Opening guidance should read as interactive orders, not a passive First Six Turns text guide.');
  ...
  assert(!rulesSource.includes('First Six Turns'), 'The old passive First Six Turns label should not ship in player-facing guide data.');
  assert(mainSource.includes('OPENING_TUTORIAL_STORAGE_KEY') && mainSource.includes('OPENING_TUTORIAL_STEPS') && mainSource.includes("id: 'move'") && mainSource.includes("id: 'build'") && mainSource.includes("id: 'attack'"), 'Opening tutorial should define success-gated move, build, and attack steps.');
  assert(mainSource.includes('recordOpeningTutorialSuccess(successCue, result)') && mainSource.includes("successCue === 'attack'") && mainSource.includes("successCue === 'build' && result?.building") && mainSource.includes("successCue === 'move'"), 'Opening tutorial progress should advance from actual successful command results.');
  assert(mainSource.includes('function openingTutorialCard') && mainSource.includes('Skip tutorial') && mainSource.includes('function openingTutorialMapHelpChip') && mainSource.includes('data-map-help-action="tutorial"') && styleSource.includes('.opening-tutorial-card') && styleSource.includes('.tutorial-directive'), 'Opening tutorial should be skippable, visible in the command rail, and echoed as an actionable battlefield chip.');
  ...
  assert(ironHandoffGuide.steps.find((step) => step.id === 'iron').label === 'Commit to an iron route', 'Guide should frame early iron as a route commitment, not a completed mine requirement.');
  assert(!ironHandoffGuide.steps.find((step) => step.id === 'iron').done, 'One opening road should not finish the iron-route milestone by itself.');
  ...
  const ironRoad = startConstruction(state, engineer.id, 'road', engineer.x, engineer.y - 1);
  assert(ironRoad.ok, ironRoad.reason || 'Guide setup iron-road commitment failed.');
  const ironRouteGuide = getFirstTurnsGuide(state);
  assert(ironRouteGuide.steps.find((step) => step.id === 'iron').done, 'Guide should recognize a committed iron road as first-turn iron progress.');
  assert(ironRouteGuide.currentId !== 'iron', 'Guide should advance away from the iron step once the route is committed.');
  assert(ironRouteGuide.steps.find((step) => step.id === 'iron').detail.includes('Iron road committed'), 'Committed iron route should tell players to keep extending it while scouting.');
...
check('canvas renderer keeps premium tactical sprites readable', () => {
  const renderSource = readProjectFile('src/render.js');
  const mainSource = readProjectFile('src/main.js');
  const normalizedRenderSource = renderSource.replace(/\r\n/g, '\n');
  ...
  assert(renderSource.includes('function drawCinematicTerrainRelief') && renderSource.includes('function drawCinematicTileGrade') && renderSource.includes('function drawCinematicTerrainSignature') && renderSource.includes('cinematicRelief: layout.tileSize >= 30'), 'The battlefield should keep a high-contrast cinematic relief pass on revealed terrain without applying it to tiny unusable tile scales.');
  assert(renderSource.includes('function drawRevealedTerrainPresenceGrade') && renderSource.includes('function terrainPresenceAlpha') && renderSource.includes('function terrainPresenceColor') && renderSource.includes('drawRevealedTerrainPresenceGrade(ctx, state, layout, sortedTiles);') && renderSource.includes("glow.addColorStop(0, 'rgba(255, 244, 190, 0.12)'") && styleSource.includes('filter: saturate(1.16) contrast(1.09)') && styleSource.includes('filter: saturate(1.20) contrast(1.12)'), 'Revealed visible terrain should get a lightweight presence grade and restrained canvas contrast so biomes stay vivid and readable on phone without darkening the overall theme.');
  ...
  assert(renderSource.includes('function drawHollowCrownCompass') && renderSource.includes('function hollowCrownDirection') && renderSource.includes('function drawHollowCrownDial') && renderSource.includes('HOLLOW CROWN') && renderSource.includes('drawHollowCrownCompass(ctx, state, layout)'), 'The primary Deadwalker objective should have a compact in-world Hollow Crown compass so portal pressure reads from the battlefield without opening panels.');
  assert(renderSource.includes('function drawDeadwalkerPressureTelegraph') && renderSource.includes('function deadwalkerPressureTelegraphData') && renderSource.includes('function drawKnownDeadwalkerThreatLane') && renderSource.includes('function drawHiddenDeadwalkerOmenFront') && renderSource.includes('function nextDeadwalkerTelegraphSurge') && renderSource.includes('drawDeadwalkerPressureTelegraph(ctx, state, layout);') && renderSource.includes('DIFFICULTY_PRESETS[state.campaign?.difficultyId]?.deadwalker'), 'Deadwalker timing and threat direction should telegraph on the battlefield as hidden omens or known threat lanes, not only in side-panel text.');
  assert(mainSource.includes('function syncTacticalPauseWarning') && mainSource.includes('lastMarchCountdown > 3 && campaign.turnsToMarch === 3') && mainSource.includes('tacticalPauseWarningTurn !== campaign.nextMarchTurn'), 'Tactical Pause should open when the Deadwalker march countdown hits 3 while avoiding repeated reopen loops.');
  assert(mainSource.includes('Deadwalker March in 3 Turns') && mainSource.includes('Pull wounded legions onto supplied tiles or Rally Banners.') && mainSource.includes('Block road lanes with shields, walls, or pact allies.') && mainSource.includes('Put archers and onagers behind the first contact line.') && styleSource.includes('.tactical-pause-kicker'), 'Tactical Pause should explain the incoming threat with concrete positioning guidance.');
  ...
  assert(renderSource.includes('const maxCameraWidth = phoneViewport ? 12 : 14') && renderSource.includes('const maxCameraHeight = phoneViewport ? 8 : 10') && renderSource.includes('Math.min(1.45, layout.tileSize * 0.052)') && renderSource.includes('Math.min(5, Math.floor(layout.tileSize * 0.15))'), 'The map should hold an adaptive close tactical camera and keep minimap scale bounded.');
  ...
  assert(mainSource.includes('function mapCanvasDprCap') && mainSource.includes('if (cssPixels >= 760000) return 1.25;') && mainSource.includes('Math.min(window.devicePixelRatio || 1, dprCap)') && mainSource.includes('const minX = Math.max(0, unit.x - def.move)') && renderSource.includes('moveCostFor(state, unit') && renderSource.includes('function reconstructReachablePath'), 'Surface and phone play should adaptively cap canvas pixel cost and use one-pass terrain-cost movement fields instead of repeated path scans.');
  assert(mainSource.includes('let pendingHoverFrame = null') && mainSource.includes('function scheduleHoverRender') && mainSource.includes('requestAnimationFrame') && mainSource.includes('cancelAnimationFrame(pendingHoverFrame)'), 'Hovering the battlefield should coalesce expensive canvas redraws into animation frames for Surface and phone performance.');
  assert(renderSource.includes('function drawTacticalActionOverlay') && renderSource.includes('function collectReachableTiles') && renderSource.includes('function drawMovementRadiusField') && renderSource.includes('function drawMovementRadiusStrategicPlate') && renderSource.includes('function drawMovementRadiusCommandSurface') && renderSource.includes('function drawMovementInteriorPips') && renderSource.includes('function drawMovementRadiusBoundary') && renderSource.includes('function drawMovementBoundaryStud') && renderSource.includes('function drawMovementTopographyCues') && renderSource.includes('function drawAttackReachTile') && renderSource.includes('function drawCommandHalo') && renderSource.includes('function drawSelectedMovementCommandAegis') && renderSource.includes('function drawCommandAegisRoute'), 'Selected units should project a clear terrain-aware movement radius, attack reach, and command focus directly onto the battlefield.');
  assert(renderSource.includes('function drawTacticalMoveMarker') && renderSource.includes('function drawMovementDestinationPedestal') && renderSource.includes('function drawMoveCostCartouche') && renderSource.includes('function drawMoveIntentSigil') && renderSource.includes('function shouldShowMoveCostCartouche') && renderSource.includes('function movementOverlayScale') && renderSource.includes('function drawRoadChevron') && renderSource.includes('function drawFootstepPair'), 'Movement destinations should use scaled in-world standards, road chevrons, selective cost cartouches, and symbol-first intent marks rather than tiny debug pips.');
  ...
  assert(renderSource.includes('const rugged = item.terrainCost > 1 || item.terrainPressure > 2.8 || item.relief > 0.45;') && renderSource.includes('const roadAnchor = item.road && (compact || item.cost <= 2 || frontier);') && renderSource.includes('const suppliedAnchor = item.supplied && (compact || item.cost <= 2 || frontier);') && renderSource.includes('const compactAnchor = compact && (frontier || punishingTerrain || item.road || item.cost <= 1);') && renderSource.includes('return frontier || punishingTerrain || (logisticMove && closeMove) || compactAnchor;') && !renderSource.includes('if (item.road || item.supplied || item.cost <= 2 || item.terrainCost > 1) return true;'), 'Movement cost cartouches should be deterministic tactical anchors for rough, road, supply, frontier, and compact decisions without becoming labels on every tile.');
  ...
  assert(renderSource.includes('function shouldStreamlineMovementOverlay') && renderSource.includes('layout.canvasWidth * layout.canvasHeight >= 1100000') && normalizedRenderSource.includes('if (!streamlinedOverlay) {\n    drawMovementBlockedApproaches'), 'Large Surface-class canvases should keep legal movement outlines fast by streamlining extra overlay passes when no route is hovered.');
  ...
  assert(renderSource.includes('function drawMiniMapSurveyChip') && renderSource.includes('if (compactInset && layout.mapWidth < 560)') && renderSource.includes('function drawHollowCrownMicroCompass') && renderSource.includes('if (compact && layout.mapWidth < 560)'), 'Phone battlefield instruments should collapse to small survey and Hollow Crown chips instead of covering the playfield with full floating panels.');
  assert(mainSource.includes('Gold outlines every legal terrain-cost move') && mainSource.includes('Gold legal moves | Blue road moves') && indexSource.includes('Gold outlines every legal terrain-cost move') && indexSource.includes('Blue outlines road-speed moves') && indexSource.includes('Laurels mark supplied ground') && indexSource.includes('Hover previews route cost') && indexSource.includes('Red rim marks attack reach'), 'The compact battlefield hint should explain tactical overlays without opening another panel.');
  ...
  assert(indexSource.includes('id="mobileIntelDrawer" class="mobile-intel-drawer"') && !indexSource.includes('id="mobileIntelDrawer" class="mobile-intel-drawer" open') && mainSource.includes('function renderMobileIntelDrawer') && mainSource.includes('function syncMobileIntelDrawer') && mainSource.includes('mobileIntelDrawerTouched') && normalizedMainSource.includes("if (state.status !== 'playing') return true;\n  return false;"), 'Mobile strategy context should collapse into a live campaign-intelligence drawer instead of forcing a long document scroll.');
  ...
  assert(mainSource.includes('function blockedScoutRecovery') && mainSource.includes('Hold while scout reports') && mainSource.includes('End turn for fresh scout') && mainSource.includes('scoutRecoveryLabel(stepId)'), 'Spent scout opening directives should provide executable hold or end-turn recovery orders instead of degrading into preview-only cards.');
  assert(mainSource.includes('function openingActionSource') && mainSource.includes('function openingActionMatchesSelection') && mainSource.includes('function focusOpeningActionSource') && mainSource.includes('selectionMismatch') && mainSource.includes('Selects the order source'), 'Opening doctrine primary actions should focus the recommended actor before allowing a different selected unit to execute that order.');
  assert(mainSource.includes('const openingAction = directive ? openingDirectiveAction(directive.current.id) : null;') && mainSource.includes('state.selectedUnitId !== openingAction.unitId') && mainSource.includes('state.selectedBuildingId !== openingAction.buildingId') && mainSource.includes('focusOpeningActionSource(openingAction)'), 'Next ready unit should prioritize the current opening order source before cycling through unrelated ready forces.');
  ...
  assert(renderSource.includes('function drawOpeningOrderRoute') && renderSource.includes('function drawOpeningOrderForeground') && renderSource.includes('function drawOpeningTargetStandard') && renderSource.includes('function openingOrderColor') && renderSource.includes('function drawOpeningHoldGroundField') && renderSource.includes('function drawOpeningHoldShieldStuds') && renderSource.includes('function drawOpeningHoldGroundStandard') && renderSource.includes('function drawOpeningHoldShieldGlyph'), 'Opening directive battlefield guidance should render as an imperial route, target standard, and foreground hold-ground stance field rather than only a side-panel instruction.');
  ...
  assert(mainSource.includes("kind: 'build'") && mainSource.includes('runCommand(buildCommand(action.unitId, action.buildingType, action.x, action.y),') && mainSource.includes('selectBuilding(placed.id)'), 'Opening engineer directives should directly start valid construction orders and focus the new structure.');
  ...
  assert(mainSource.includes('function openingBlockedInfrastructureAction') && mainSource.includes('Scout while engineer readies') && mainSource.includes('Hold while engineer readies') && mainSource.includes("kind: 'fortify'") && mainSource.includes('runCommand(fortifyCommand(action.unitId),') && mainSource.includes("kind: 'end-turn'") && mainSource.includes('requestEndTurn(true)'), 'Blocked opening infrastructure directives should scout with ready recon, spend idle formations, and then advance the turn instead of leaving a preview-only dead end.');
  assert(mainSource.includes('function recommendedIronOrder') && mainSource.includes("return blockedBuilderRecovery(engineer, 'Hill Mine route', 'Iron road');") && mainSource.includes('function bestIronMineTarget') && mainSource.includes('function bestIronRoadAdvance') && mainSource.includes('function ironRouteProgressLabel') && mainSource.includes('route left to') && mainSource.includes('Extend iron road') && mainSource.includes('Advance along the iron road') && mainSource.includes('Hold for iron route') && mainSource.includes('Iron road'), 'The iron opening directive should pursue a mine route with visible route-left progress and a clear engineer-refresh handoff instead of falling back to generic road or tower construction.');
  ...
  assert(mainSource.includes('function isCompactCommandRailMode') && mainSource.includes('build-orders build-command-drawer') && mainSource.includes('!isCompactCommandRailMode()') && styleSource.includes('.build-command-drawer'), 'Engineer construction orders should collapse into a compact drawer on Surface and phone command rails while staying available for deeper planning.');
  ...
  assert(renderSource.includes('function buildSiteSurvey') && renderSource.includes('function drawBuildSurveyConnectors') && renderSource.includes('function drawBuildSitePlate') && renderSource.includes('function drawBuildSiteBlueprint') && renderSource.includes('function drawBuildSiteBadge') && renderSource.includes('function buildSiteBadgeAnchor') && renderSource.includes('buildSiteShortLabel'), 'Active build mode should project a premium in-world placement survey with readable site plates, blueprint icons, and touch-sized labels.');
  assert(renderSource.includes("const placementFocusMode = state.mode.type === 'build';") && normalizedRenderSource.includes('if (!placementFocusMode) {\n    drawDiplomacyOpportunityRoute') && normalizedRenderSource.includes('drawMissionFocus(ctx, state, layout, missionFocusOverlay);\n  }'), 'Active build mode should suppress unrelated mission, opening-order, and diplomacy routes so placement markers have visual priority.');
  ...
  assert(mainSource.includes('Gold outlines every legal terrain-cost move') && mainSource.includes('Blue outlines road-speed moves') && mainSource.includes('Gold legal moves | Blue road moves'), 'The live map help should explain legal movement outlines and road-speed movement at a glance.');
  ...
  assert(mainSource.includes('const idealHeight = width * (compactViewport ? 1.02 : 0.8)'), 'The default battlefield should use a taller map-first command viewport on phones without increasing desktop canvas cost.');
  ...
  assert(styleSource.includes('Praetorium polish') && styleSource.includes('font-size: clamp(0.42rem, 2.35vw, 0.50rem)') && styleSource.includes('.map-lens-bar .lens-label') && styleSource.includes('overflow: hidden'), 'Phone-first chrome should use the bright final polish layer and prevent clipped map lens buttons.');
  assert(mainSource.includes('function compactResourceLabel') && mainSource.includes("resourceChip('population', compactChrome ? 'Pop' : 'Population'") && mainSource.includes("influence: 'Inf'") && mainSource.includes("morale: 'Mor'") && styleSource.includes('Mobile empire ribbon') && styleSource.includes('flex-wrap: nowrap') && styleSource.includes('overflow-x: auto') && styleSource.includes('scroll-snap-type: x proximity') && styleSource.includes('.resourcebar::-webkit-scrollbar') && styleSource.includes('.resource.population'), 'Mobile economy resources should collapse into a one-line scrollable empire ribbon with compact labels so the battlefield, not dashboard chrome, owns the first viewport.');
  assert(mainSource.includes('class="selection-dossier"') && mainSource.includes('class="selection-dossier-summary"') && mainSource.includes("selectionPanel.classList.toggle('selection-panel-collapsed'") && styleSource.includes('#selectionPanel.selection-panel-collapsed') && styleSource.includes('.selection-dossier-summary::after'), 'Mobile selected-unit dossiers should collapse behind a compact summary because the order rail already carries active command context.');
  assert(mainSource.includes('function mobileCollapsibleActionSection') && mainSource.includes('function appendOrderContent') && mainSource.includes("mobile-collapsible-order ${className}") && styleSource.includes('.mobile-collapsible-summary') && styleSource.includes('.mobile-collapsible-order[open] .mobile-collapsible-summary::after') && styleSource.includes('.mobile-collapsible-order .mobile-collapsible-body > .order-section-head'), 'Mobile secondary unit and campaign orders should collapse behind compact drawers so the battlefield and primary orders stay dominant.');
  assert(mainSource.includes('function movementFieldMapHelpChip') && mainSource.includes('function selectedMovementFieldSummary') && mainSource.includes('Move ${escapeHtml(String(summary.move))}') && mainSource.includes('summary.reachable') && mainSource.includes('summary.rough') && styleSource.includes('.move-field-directive'), 'Selected ready units should surface a compact movement-field summary so terrain-adjusted movement radius is readable on phones.');
  ...
  assert(mainSource.includes('function tacticalCounselCard') && mainSource.includes('function unitTacticalCounsel') && mainSource.includes('function bestReachableTacticalStance') && mainSource.includes('function visibleHostilePressure') && mainSource.includes('actionPanel.appendChild(counselCard)') && styleSource.includes('.tactical-counsel') && styleSource.includes('.tactical-counsel-tags') && styleSource.includes('.tactical-counsel-stats'), 'Selected units and structures should expose compact terrain, threat, and stance counsel so tactical depth is readable without slowing play.');
  ...
  assert(styleSource.includes('.map-intel') && styleSource.includes('.map-intel-stats') && styleSource.includes('pointer-events: none') && styleSource.includes('.map-intel.order') && styleSource.includes('.map-intel.order.diplomacy') && styleSource.includes('.map-intel.order .map-intel-stats span:last-child'), 'Field intel should be a compact non-blocking battlefield overlay with readable opening and diplomacy command chips.');
  ...
  assert(indexSource.includes('id="mapTurnReport"') && mainSource.includes('function renderMapTurnReport') && mainSource.includes('const glance = turnReport.metrics.map') && mainSource.includes('map-turn-report-deltas') && mainSource.includes('map-turn-report-notes') && mainSource.includes('map-turn-report-next-command') && mainSource.includes('turnReportContinueLabel(nextCommand)') && mainSource.includes('mapTurnReport?.addEventListener') && mainSource.includes('function handleTurnReportAction') && styleSource.includes('.map-turn-report:not([hidden])') && styleSource.includes('.map-turn-report-deltas') && styleSource.includes('.map-turn-report-notes') && styleSource.includes('.map-turn-report-next-command'), 'Mobile turn reports should surface complete strategic battlefield chips with direct next-command actions instead of forcing the map offscreen.');
  ...
  assert(mainSource.includes('function captureTurnSnapshot') && mainSource.includes('function buildTurnReport') && mainSource.includes('function turnReportCard') && mainSource.includes('function turnReportNextCommandState') && mainSource.includes('function turnReportNextCommandMarkup') && mainSource.includes('function turnReportContinueLabel') && mainSource.includes('function focusTurnReportOnMobile') && mainSource.includes('scrollBattlefieldIntoView();') && mainSource.includes('turn-report-detail-drawer') && mainSource.includes('turn-report-glance'), 'End-turn resolution should show a structured, mobile-compact campaign report and preserve one-more-turn command momentum.');
  ...
  assert(styleSource.includes('.turn-report') && styleSource.includes('.turn-report-metrics') && styleSource.includes('.turn-report-notes') && styleSource.includes('.turn-report-detail-drawer summary') && styleSource.includes('.turn-report-glance') && styleSource.includes('.turn-report-actions') && styleSource.includes('.turn-report-next-command') && styleSource.includes('text-overflow: ellipsis'), 'Turn reports need readable strategy-card styling with compact mobile disclosure, next-command momentum, and overflow-safe labels.');
  ...
  assert(mainSource.includes('Turn ${state.turn} resolved - ${pressureToast}, ${readyToast}.') && mainSource.includes('const pressureToast = deadPressureDelta > 0') && mainSource.includes('function compactToastText') && styleSource.includes('.toast.bad') && styleSource.includes('color: #661b15') && styleSource.includes('top: calc(env(safe-area-inset-top') && styleSource.includes('max-width: min(16.75rem') && styleSource.includes('overflow-wrap: anywhere') && styleSource.includes('-webkit-line-clamp: 2'), 'Toast confirmations should stay readable, concise, and clear of mobile command cards in the light theme.');
...
check('combat can damage enemies and protects portal before boss death', () => {
  ...
  assert(renderSource.includes('function drawStrategicImpact') && renderSource.includes('function drawStrategicImpactRings') && renderSource.includes('fitStrategicImpactTitle'), 'Successful pacts and pact orders should draw a treaty impact marker on the target tile.');
  assert(styleSource.includes('.battle-impact') && styleSource.includes('.battle-impact.good') && styleSource.includes('.battle-impact.bad') && styleSource.includes('.battle-impact.strategy'), 'Combat and strategic impact cards need readable tone styling.');
...
check('No Canvas 2D calls outside PixiJS abstraction', () => {
  const allowed = new Set([
    path.join(root, 'src/render.js'),
    path.join(root, 'src/engine/pixi-renderer.js')
  ]);
  for (const file of listFiles(path.join(root, 'src'))) {
    if (allowed.has(file)) continue;
    const text = readFileSync(file, 'utf8');
    assert(!/getContext\(\s*['"]2d['"]\s*\)/.test(text), `${path.relative(root, file)} must not call Canvas 2D directly.`);
  }
  assert(existsSync(path.join(root, 'src/engine/pixi-renderer.js')), 'Pixi renderer backbone missing.');
});

check('All state mutations route through Command.execute()', () => {
  const commandsSource = readProjectFile('src/engine/commands.js');
  const mainSource = readProjectFile('src/main.js');
...
check('JSON content validates against schema', () => {
  assert(dataFilesExist(), 'data/ JSON content files are missing.');
...
check('Undo stack reverses any player action without desync', () => {
  ...
  const path = findPath(state, scout, startX + 1, startY, UNIT_TYPES.scout.move);
  assert(path, 'Undo test needs a legal scout move.');
  const moveResult = history.execute(moveCommand(scout.id, startX + 1, startY), state);
  assert(moveResult.ok, moveResult.reason || 'Move command failed.');
  assert(scout.x === startX + 1, 'Move did not apply.');
  const undoResult = history.undo(state);
  assert(undoResult.ok, undoResult.reason || 'Undo failed.');
  const scoutAfterUndo = state.units.find((u) => u.id === scout.id);
  assert(scoutAfterUndo.x === startX && scoutAfterUndo.y === startY, 'Undo did not restore scout position.');
  const city = state.buildings.find((b) => b.faction === 'olundar' && b.type === 'city');
  const beforeFood = state.factions.olundar.resources.food;
  const trainResult = history.execute(trainCommand(city.id, 'scout'), state);
  assert(trainResult.ok, trainResult.reason || 'Train command failed.');
  assert(state.factions.olundar.resources.food < beforeFood, 'Train did not spend food.');
  history.undo(state);
  assert(state.factions.olundar.resources.food === beforeFood, 'Undo did not restore resources after train.');
  const engineer = state.units.find((u) => u.type === 'engineer');
  const buildResult = history.execute(buildCommand(engineer.id, 'road', engineer.x, engineer.y), state);
...
check('PixiJS engine foundation is present', () => {
  ...
  assert(pkg.dependencies?.['pixi.js'], 'pixi.js dependency missing from package.json.');
  const renderer = readProjectFile('src/engine/pixi-renderer.js');
  const camera = readProjectFile('src/engine/camera.js');
  const particles = readProjectFile('src/engine/particles.js');
  assert(renderer.includes('IMPERIAL_PALETTE') && renderer.includes('drawVignette'), 'Pixi renderer must include imperial lighting.');
  assert(renderer.includes('function pixiTileWindow') && renderer.includes('function forEachPixiWindowTile') && renderer.includes('function isScreenPointInViewport'), 'Pixi renderer must cull overlay work to the tactical camera and pixel viewport.');
  assert(renderer.includes('state.visible') && renderer.includes('map.tiles') && renderer.includes('particles.draw(g, viewport?.pixel)'), 'Pixi overlay must read the live map/visibility state and skip off-screen particles.');
...
check('audio mixer exposes per-bus volume controls', () => {
  const audio = readProjectFile('src/engine/audio.js');
  const settings = readProjectFile('src/settings.js');
  const main = readProjectFile('src/main.js');
  const tensionTargets = musicLayerTargetsForMode('tension');
  const combatTargets = musicLayerTargetsForMode('combat');
  const plainsAmbience = ambientTileTargetsForContext({ terrain: 'plains' });
  const marshAmbience = ambientTileTargetsForContext({ terrain: 'marsh' });
  const blightAmbience = ambientTileTargetsForContext({ terrain: 'blight', blight: 5 });
  const constructionAmbience = ambientTileTargetsForContext({ terrain: 'plains', nearConstruction: true });
  assert(audio.includes("buses.sfx") && audio.includes("buses.ambient") && audio.includes("buses.music") && audio.includes("buses.ui"), 'Audio mixer must define SFX, Ambient, Music, and UI buses.');
  assert(audio.includes('updateDynamicMusic') && audio.includes('crossfadeMusicLayers'), 'Audio must support dynamic music layers.');
  assert(audio.includes('cancelAndHoldAtTime') && audio.includes('linearRampToValueAtTime'), 'Dynamic music should schedule cancellable crossfades instead of abrupt gain cuts.');
  assert(MUSIC_CROSSFADE_SECONDS >= 1 && MUSIC_CROSSFADE_SECONDS <= 2, 'Music layer crossfades should be smooth without feeling delayed.');
  assert(tensionTargets.exploration > 0.003 && tensionTargets.tension > tensionTargets.exploration, 'Tension music should layer threat over the exploration bed.');
  assert(combatTargets.combat > combatTargets.tension && combatTargets.tension > combatTargets.exploration, 'Combat music should crossfade through escalating layer targets.');
  assert(audio.includes('updateAmbientTileSound') && main.includes('ambientAudioContextForFocus') && main.includes('nearbyAmbientConstruction'), 'Gameplay focus should drive contextual ambient tile sound.');
  assert(AMBIENT_TILE_FADE_SECONDS >= 0.5 && AMBIENT_TILE_FADE_SECONDS <= 1.5, 'Ambient tile fades should respond quickly without clicking.');
  assert(plainsAmbience.wind > plainsAmbience.water && plainsAmbience.wind > plainsAmbience.crows, 'Plains ambience should prioritize wind.');
  assert(marshAmbience.water > marshAmbience.wind && marshAmbience.water > marshAmbience.crows, 'Marsh and water ambience should prioritize water.');
  assert(blightAmbience.crows > blightAmbience.wind && blightAmbience.crows > blightAmbience.water, 'Blight ambience should prioritize crows.');
