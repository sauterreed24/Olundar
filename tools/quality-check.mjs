...
check('canvas renderer keeps premium tactical sprites readable', () => {
  ...
  assert(renderSource.includes('function drawTopographicContourInk') && renderSource.includes('function drawRiverbankHighlights'), 'The battlefield should add topographic contour ink and riverbank lighting for crafted-map depth.');
  ...
  assert(renderSource.includes('function drawMiniMapCompass') && renderSource.includes('function roundRectPath') && renderSource.includes('layout.camera.width * scale') && renderSource.includes('compactInset ? 3 : 5'), 'The minimap should render as a polished responsive strategic inset with camera bounds, not a flat debug square.');
  ...
  assert(renderSource.includes('Math.max(9, Math.min(14, revealedWidth + 2))') && renderSource.includes('compactInset ? 3 : 5') && renderSource.includes('compactInset ? 0.11 : 0.15'), 'The map should hold a close tactical camera and keep minimap scale bounded.');
  assert(renderSource.includes('function drawTacticalActionOverlay') && renderSource.includes('function collectReachableTiles') && renderSource.includes('function drawAttackReachTile') && renderSource.includes('function drawCommandHalo'), 'Selected units should project movement, attack reach, and command focus directly onto the battlefield.');
  assert(indexSource.includes('Gold/blue tiles show movement') && indexSource.includes('Red rim marks attack reach'), 'The compact battlefield hint should explain tactical overlays without opening another panel.');
  ...
  assert(mainSource.includes('function openingBlockedInfrastructureAction') && mainSource.includes("kind: 'fortify'") && mainSource.includes('fortifyUnit(state, action.unitId)') && mainSource.includes("kind: 'end-turn'") && mainSource.includes('requestEndTurn(true)'), 'Blocked opening infrastructure directives should spend idle formations and then advance the turn instead of leaving a preview-only dead end.');
  ...
  assert(renderSource.includes('function drawUnitPlinth') && renderSource.includes('function drawUnitRoleDevice') && renderSource.includes('function drawActedUnitVeil'), 'Battlefield units should have premium bases, role devices, and readable action-state treatment.');
  ...
  assert(mainSource.includes('function selectionStatMarkup') && mainSource.includes('function orderButton') && mainSource.includes('campaign-tools'), 'The command rail should expose structured stat chips, hierarchy, and quieter campaign utilities.');
  assert(styleSource.includes('.command-stats') && styleSource.includes('.stat-chip') && styleSource.includes('.order-button.primary') && styleSource.includes('.campaign-tools'), 'The command rail should look like a polished imperial control surface rather than generic form buttons.');
  ...
  assert(styleSource.includes('.map-intel') && styleSource.includes('.map-intel-stats') && styleSource.includes('pointer-events: none'), 'Field intel should be a compact non-blocking battlefield overlay.');
  assert(mainSource.includes('function captureTurnSnapshot') && mainSource.includes('function buildTurnReport') && mainSource.includes('function turnReportCard') && mainSource.includes('function focusTurnReportOnMobile'), 'End-turn resolution should show a structured campaign report instead of a generic toast.');
  assert(styleSource.includes('.turn-report') && styleSource.includes('.turn-report-metrics') && styleSource.includes('.turn-report-notes'), 'Turn reports need readable strategy-card styling.');
