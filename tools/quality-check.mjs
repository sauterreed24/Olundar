...
check('canvas renderer keeps premium tactical sprites readable', () => {
  ...
  assert(renderSource.includes('function drawMiniMapCompass') && renderSource.includes('function roundRectPath') && renderSource.includes('layout.camera.width * scale') && renderSource.includes('compactInset ? 4 : 6'), 'The minimap should render as a polished responsive strategic inset with camera bounds, not a flat debug square.');
  ...
  assert(renderSource.includes('Math.max(10, Math.min(16, revealedWidth + 2))') && renderSource.includes('compactInset ? 4 : 6') && renderSource.includes('compactInset ? 0.13 : 0.18'), 'The map should hold a close tactical camera and keep minimap scale bounded.');
  ...
  assert(styleSource.includes('.next-directive') && styleSource.includes('.opening-doctrine') && styleSource.includes('.doctrine-actions'), 'Opening directive surfaces need compact readable battlefield and command styling.');
