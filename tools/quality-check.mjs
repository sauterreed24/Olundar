...
check('canvas renderer keeps premium tactical sprites readable', () => {
  ...
  assert(mainSource.includes('Turn ${state.turn} resolved - ${pressureToast}, ${readyToast}.') && mainSource.includes('const pressureToast = deadPressureDelta > 0') && styleSource.includes('.toast.bad') && styleSource.includes('color: #661b15') && styleSource.includes('top: calc(env(safe-area-inset-top') && styleSource.includes('max-width: min(19rem') && styleSource.includes('-webkit-line-clamp: 2'), 'Toast confirmations should stay readable, concise, and clear of mobile command cards in the light theme.');
