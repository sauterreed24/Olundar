...
check('canvas renderer keeps premium tactical sprites readable', () => {
  ...
  assert(mainSource.includes("kind: 'build'") && mainSource.includes('createBuildCommand(action.unitId, action.buildingType, action.x, action.y)') && mainSource.includes('selectBuilding(placed.id)'), 'Opening engineer directives should directly start valid construction orders and focus the new structure.');
  ...
  assert(mainSource.includes('function musterReadyCard') && mainSource.includes('function musterReadyOrders') && mainSource.includes('function preferredMusterOrder') && mainSource.includes('function musterReadyButton') && mainSource.includes('function queueMusterOrder') && mainSource.includes('Muster Ready') && mainSource.includes('createTrainCommand(building.id, unitType)') && styleSource.includes('.muster-ready-card') && styleSource.includes('.muster-ready-button') && styleSource.includes('.muster-ready-grid') && styleSource.includes('.muster-ready-cost'), 'Available training queues should surface as direct muster commands instead of requiring players to discover city production by selecting buildings first.');
...
check('All state mutations route through Command.execute()', () => {
  ...
  assert(commandsSource.includes("case 'crisis'") && commandsSource.includes("case 'fieldOrder'"), 'Command.execute must cover crisis and field orders.');
