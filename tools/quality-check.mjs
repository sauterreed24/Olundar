...
check('canvas renderer keeps premium tactical sprites readable', () => {
  ...
  assert(mainSource.includes('function recommendedIronOrder') && mainSource.includes("return blockedBuilderRecovery(engineer, 'Hill Mine route', 'Claim iron');") && mainSource.includes('function bestIronMineTarget') && mainSource.includes('function bestIronRoadAdvance') && mainSource.includes('function ironRouteProgressLabel') && mainSource.includes('route left to') && mainSource.includes('Extend iron road') && mainSource.includes('Advance along the iron road') && mainSource.includes('Hold for iron claim') && mainSource.includes('Claim iron'), 'The iron opening directive should pursue a mine route with visible route-left progress and a clear engineer-refresh handoff instead of falling back to generic road or tower construction.');
