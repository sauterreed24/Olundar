...
check('canvas renderer keeps premium tactical sprites readable', () => {
  ...
  assert(styleSource.includes('.map-intel') && styleSource.includes('.map-intel-stats') && styleSource.includes('pointer-events: none') && styleSource.includes('.map-intel.order') && styleSource.includes('.map-intel.order .map-intel-stats span:last-child'), 'Field intel should be a compact non-blocking battlefield overlay with readable opening command chips.');
