...
check('canvas renderer keeps premium tactical sprites readable', () => {
  ...
  assert(mainSource.includes('Turn ${state.turn} resolved - ${pressureToast}, ${readyToast}.') && mainSource.includes('const pressureToast = deadPressureDelta > 0') && styleSource.includes('.toast.bad') && styleSource.includes('color: #661b15') && styleSource.includes('bottom: calc(env(safe-area-inset-bottom') && styleSource.includes('-webkit-line-clamp: 2'), 'Toast confirmations should stay readable, concise, and clear of the mobile header in the light theme.');
