...
check('canvas renderer keeps premium tactical sprites readable', () => {
  ...
  assert(mainSource.includes('function compactPactCommandDetail') && mainSource.includes('compact-pact-command') && styleSource.includes('.compact-pact-command') && styleSource.includes('.compact-pact-command .pact-actions'), 'Active pact field commands should collapse into compact retasking controls on phone and Surface rails instead of pushing the player into a long alliance panel.');
