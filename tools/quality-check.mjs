...
check('canvas renderer keeps premium tactical sprites readable', () => {
  ...
  assert(renderSource.includes('function drawMiniMapCompass') && renderSource.includes('function roundRectPath') && renderSource.includes('layout.camera.width * scale') && renderSource.includes('compactInset ? 2 : 5'), 'The minimap should render as a polished responsive strategic inset with camera bounds, not a flat debug square.');
  ...
  assert(renderSource.includes('Math.max(9, Math.min(14, revealedWidth + 2))') && renderSource.includes('compactInset ? 2 : 5') && renderSource.includes('compactInset ? 0.085 : 0.15'), 'The map should hold a close tactical camera and keep minimap scale bounded.');
  ...
  assert(indexSource.includes('Standards mark key moves') && indexSource.includes('Hover previews route cost') && indexSource.includes('Red rim marks attack reach'), 'The compact battlefield hint should explain tactical overlays without opening another panel.');
  ...
  assert(mainSource.includes('function focusOpeningFollowThrough') && mainSource.includes('focusOpeningFollowThrough(previousStepId)'), 'Successful opening orders should focus the next recommended actor instead of leaving players on a spent unit.');
