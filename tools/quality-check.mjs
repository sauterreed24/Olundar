...
check('canvas renderer keeps premium tactical sprites readable', () => {
  ...
  assert(mainSource.includes('Math.min(window.devicePixelRatio || 1, 2)') && mainSource.includes('const minX = Math.max(0, unit.x - def.move)') && renderSource.includes('const minX = Math.max(0, unit.x - move)'), 'Surface and phone play should cap canvas pixel cost and avoid whole-map movement recommendation scans.');
  assert(renderSource.includes('function drawTacticalActionOverlay') && renderSource.includes('function collectReachableTiles') && renderSource.includes('function drawMovementRadiusField') && renderSource.includes('function drawMovementRadiusBoundary') && renderSource.includes('function drawMovementTopographyCues') && renderSource.includes('function drawAttackReachTile') && renderSource.includes('function drawCommandHalo'), 'Selected units should project a clear terrain-aware movement radius, attack reach, and command focus directly onto the battlefield.');
  ...
  assert(mainSource.includes('function scrollOpeningFollowThroughRail') && mainSource.includes("['scout', 'contact', 'front']") && mainSource.includes('function resetCommandRailScroll') && mainSource.includes('resetCommandRailScroll();') && mainSource.includes('function scrollBattlefieldIntoView') && mainSource.includes('const offset = Math.ceil(topbar?.getBoundingClientRect().height || 0) + 24;') && mainSource.includes('window.scrollY + battlefield.getBoundingClientRect().top - offset') && mainSource.includes('scrollOpeningFollowThroughRail(previousStepId)'), 'Scout and contact follow-through should preserve battlefield focus after selecting the next actor without clipping desktop command context.');
  ...
  assert(mainSource.includes('function captureTurnSnapshot') && mainSource.includes('function buildTurnReport') && mainSource.includes('function turnReportCard') && mainSource.includes('function focusTurnReportOnMobile') && mainSource.includes('turn-report-detail-drawer') && mainSource.includes('turn-report-glance'), 'End-turn resolution should show a structured, mobile-compact campaign report instead of a generic toast.');
  assert(mainSource.includes('function continueTurnReportOrders') && mainSource.includes('data-action="continue-turn-orders"') && mainSource.includes('focusOpeningFollowThrough(previousStepId);'), 'End-turn reports should let players continue directly to the next recommended opening order.');
