...
check('canvas renderer keeps premium tactical sprites readable', () => {
  ...
  assert(renderSource.includes('function drawTacticalMoveMarker') && renderSource.includes('function drawMovementDestinationPedestal') && renderSource.includes('function drawMoveCostCartouche') && renderSource.includes('function movementOverlayScale') && renderSource.includes('function drawRoadChevron') && renderSource.includes('function drawFootstepPair'), 'Movement destinations should use scaled in-world standards, road chevrons, and cost cartouches rather than tiny debug pips.');
  ...
  assert(renderSource.includes('const rugged = item.terrainCost > 1 || item.terrainPressure > 2.8 || item.relief > 0.45') && renderSource.includes('const roadAnchor = item.road') && renderSource.includes('const suppliedAnchor = item.supplied') && renderSource.includes('const frontierSurvey = frontier && tileNoise') && !renderSource.includes('if (item.road || item.supplied || item.cost <= 2 || item.terrainCost > 1) return true;'), 'Movement cost cartouches should be selective tactical anchors, not numeric labels on every supplied or road tile.');
