...
check('canvas renderer keeps premium tactical sprites readable', () => {
  ...
  assert(renderSource.includes('const rugged = (item.terrainCost > 1 || item.terrainPressure > 2.8 || item.relief > 0.45) && tileNoise') && renderSource.includes('const roadAnchor = item.road && (item.cost <= 1') && renderSource.includes('const suppliedAnchor = item.supplied && item.cost <= 1') && renderSource.includes('const frontierSurvey = frontier && tileNoise') && renderSource.includes('const hardFrontier = frontier && (punishingTerrain || tileNoise') && !renderSource.includes('if (item.road || item.supplied || item.cost <= 2 || item.terrainCost > 1) return true;'), 'Movement cost cartouches should be rare tactical anchors, not numeric labels on every supplied, road, or frontier tile.');
  ...
  assert(mainSource.includes('data-map-help-action="opening"') && mainSource.includes('data-map-help-action="deadwalker"') && mainSource.includes("mapHelp?.addEventListener('click'") && mainSource.includes('focusOpeningDirective(directive.current.id)') && mainSource.includes('focusDeadwalkerIntent(intent)') && styleSource.includes('.map-help button') && styleSource.includes('.map-help button:focus-visible'), 'Battlefield help chips should be touch-accessible controls for the next order and Deadwalker pressure, not passive labels.');
  assert(mainSource.includes('function selectedCommandStrip') && mainSource.includes('function unitCommandRole') && mainSource.includes('actionPanel.appendChild(commandStrip)') && mainSource.includes('const compactRail = isCompactCommandRailMode();') && mainSource.includes('const directiveSourceCard = openingDirectiveSourceCard();') && mainSource.includes('const tacticalMoveCard = tacticalMoveOrdersCard(selectedUnit);') && mainSource.includes('const deadwalkerIntent = deadwalkerIntentCard();') && mainSource.includes('const readyForcesCard = activeReadyForcesCard(selectedUnit);') && mainSource.includes('const musterCard = musterReadyCard(selectedBuilding);') && mainSource.includes('const endTurnReview = endTurnReviewCard();') && mainSource.includes('const priorityCommandCards = [placementCard, directiveSourceCard, tacticalMoveCard, doctrineCard, deadwalkerIntent, readyForcesCard, musterCard, endTurnReview, envoyCard, pactCommandCard].filter(Boolean);') && normalizedMainSource.includes('if (compactRail) {\n    for (const card of priorityCommandCards) actionPanel.appendChild(card);\n    if (counselCard) actionPanel.appendChild(counselCard);') && styleSource.includes('.mobile-command-strip') && styleSource.includes('.mobile-command-strip-stats'), 'Mobile order rail should keep selected context visible while putting the recommended order source, terrain-aware tactical moves, immediate guided orders, Deadwalker intent, the ready-force queue, available musters, and pending end-turn reviews before tactical counsel on compact rails.');
