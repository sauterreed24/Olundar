...
check('canvas renderer keeps premium tactical sprites readable', () => {
  ...
  assert(mainSource.includes('function musterReadyCard') && mainSource.includes('function musterReadyOrders') && mainSource.includes('function preferredMusterOrder') && mainSource.includes('function musterReadyButton') && mainSource.includes('function queueMusterOrder') && mainSource.includes('Muster Ready') && mainSource.includes('startTraining(state, building.id, unitType)') && styleSource.includes('.muster-ready-card') && styleSource.includes('.muster-ready-button') && styleSource.includes('.muster-ready-grid') && styleSource.includes('.muster-ready-cost'), 'Available training queues should surface as direct muster commands instead of requiring players to discover city production by selecting buildings first.');
