...
check('canvas renderer keeps premium tactical sprites readable', () => {
  ...
  assert(mainSource.includes('const dprCap = compactViewport ? 1.6 : 2;') && mainSource.includes('Math.min(window.devicePixelRatio || 1, dprCap)') && mainSource.includes('const minX = Math.max(0, unit.x - def.move)') && renderSource.includes('const minX = Math.max(0, unit.x - move)'), 'Surface and phone play should cap canvas pixel cost and avoid whole-map movement recommendation scans.');
  assert(renderSource.includes('function drawTacticalActionOverlay') && renderSource.includes('function collectReachableTiles') && renderSource.includes('function drawMovementRadiusField') && renderSource.includes('function drawMovementInteriorPips') && renderSource.includes('function drawMovementRadiusBoundary') && renderSource.includes('function drawMovementTopographyCues') && renderSource.includes('function drawAttackReachTile') && renderSource.includes('function drawCommandHalo'), 'Selected units should project a clear terrain-aware movement radius, attack reach, and command focus directly onto the battlefield.');
  ...
  assert(mainSource.includes('function scrollOpeningFollowThroughRail') && mainSource.includes("'training', 'engineer', 'iron', 'turn-report'") && mainSource.includes('function resetCommandRailScroll') && mainSource.includes('resetCommandRailScroll();') && mainSource.includes('function scrollBattlefieldIntoView') && mainSource.includes('const offset = Math.ceil(topbar?.getBoundingClientRect().height || 0) + 24;') && mainSource.includes('window.scrollY + battlefield.getBoundingClientRect().top - offset') && mainSource.includes('scrollOpeningFollowThroughRail(previousStepId)') && mainSource.includes('if (ok) scrollOpeningFollowThroughRail(previousStepId);'), 'Guided opening follow-through should preserve battlefield focus after selecting the next actor without clipping desktop command context.');
