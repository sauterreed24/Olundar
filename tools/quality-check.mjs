...
check('canvas renderer keeps premium tactical sprites readable', () => {
  ...
  assert(indexSource.includes('id="mapTurnReport"') && mainSource.includes('function renderMapTurnReport') && mainSource.includes('const glance = turnReport.metrics.map') && mainSource.includes('map-turn-report-deltas') && mainSource.includes('map-turn-report-notes') && mainSource.includes('mapTurnReport?.addEventListener') && mainSource.includes('function handleTurnReportAction') && styleSource.includes('.map-turn-report:not([hidden])') && styleSource.includes('.map-turn-report-deltas') && styleSource.includes('.map-turn-report-notes'), 'Mobile turn reports should surface complete strategic battlefield chips with direct actions instead of forcing the map offscreen.');
