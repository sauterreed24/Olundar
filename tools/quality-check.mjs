...
check('canvas renderer keeps premium tactical sprites readable', () => {
  ...
  assert(renderSource.includes("const placementFocusMode = state.mode.type === 'build';") && renderSource.includes('if (!placementFocusMode) {\n    drawDiplomacyOpportunityRoute') && renderSource.includes('drawMissionFocus(ctx, state, layout, missionFocusOverlay);\n  }'), 'Active build mode should suppress unrelated mission, opening-order, and diplomacy routes so placement markers have visual priority.');
