...
- Aftermath Missions that place crisis follow-through tasks on the map, such as escort roads, repair scars, raider trails, and accord routes, with visible site art, focus controls, route previews, focused route overlays, route chains, and terrain-specific rewards.
...
- Aftermath missions must be generated by selected aftermath rulings, appear on the Missions lens with distinct canvas site markers and card-level focus controls, preview the recommended eligible unit and same-turn reachability, draw a focused route overlay from revealed path data, complete through unit movement, expose spawned sites and route-chain steps, and pay terrain-specific rewards.
...
1. Add mission dispatch prompts that offer a single clear move command when the focused route is reachable this turn.
