...
- Aftermath Missions that place crisis follow-through tasks on the map, such as escort roads, repair scars, raider trails, and accord routes, with visible site art, focus controls, route previews, route chains, and terrain-specific rewards.
...
- When **Aftermath Missions** appear, use **Focus** on a mission card or the Missions lens to inspect the marked site; route previews name the recommended unit, whether it can complete the task this turn, and **Unit** jumps to that unit; camps, raider sites, accord waystations, and repair scars have distinct map art, some routes reveal a second waypoint, and each site pays a terrain-specific reward.
...
- Aftermath missions must be generated by selected aftermath rulings, appear on the Missions lens with distinct canvas site markers and card-level focus controls, preview the recommended eligible unit and same-turn reachability, complete through unit movement, expose spawned sites and route-chain steps, and pay terrain-specific rewards.
...
1. Draw focused mission route overlays on the map so the recommended unit path is visible, not just described on the card.
